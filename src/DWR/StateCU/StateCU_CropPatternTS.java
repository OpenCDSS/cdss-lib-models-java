...
/**
The StateCU_CropPatternTS class is used to hold crop time series data (acreage/area) used in StateDMI for CDS files.
Each instance has an identifier, which will match a StateCU_Location identifier,
and a list of time series for various crops that are associated with the CU Location for a period of time.
...
/**
Total acres (total of all crops) for each year in the period.
This is computed from the time series in __tslist when crop values are set.
*/
...
/**
Internal data that is used to set/get data so new DateTime objects don't need to be created each time.
Only the year is manipulated.
*/
private DateTime __temp_DateTime = new DateTime();

/**
List of crop types for the time series.  This is consistent with the data sub-types for the time series.
The list is maintained to simplify use at output.
*/
...
/**
Construct a new StateCU_CropPatternTS object for the specified CU Location identifier.
This is typically only called by readStateCUFile().
@param id StateCU_Location identifier.
@param date1 Starting date of period.  Specify with year 0 if an average annual data set.
@param date2 Ending date of period.  Specify with year 0 if an average annual data set.
@param units Data units for the acreage.
...
This is used to store raw data (e.g., from HydroBase) so that later filling and checks can more easily be performed.
@param parcel StateCU_Parcel to add.
*/
public void addParcel ( StateCU_Parcel parcel ) {
	__parcelList.add ( parcel );
	String routine = getClass().getSimpleName() + ".addParcel";
...
@param overwrite If false, the time series is only added if it does not already exist.
If true, the time series is added regardless and replaces the existing time series.
...
public YearTS addTS ( String cropName, boolean overwrite, boolean sortByCrop ) {
	...
	if ( pos < 0 ) {
		...
		if ( !sortByCrop || (this.__tslist.size() == 0) ) {
			...
			// Find the insert position.  If previously added time series were sorted, the following works.
			int posInsert = -1;
...
@return the crop acreage for the given year.
Return -999.0 if the crop is not found or the requested year is outside the data period.
@param crop_name Name of the crop, only the crop name without the leading "CropArea-".
@param year Year to retrieve data.
@param return_fraction If true, return the acreage as a fraction (0.0 to 1.0) of the total.  If false, return the acreage.
*/
...
@return a list of distinct crop names, determined from dataList.  A non-null list is guaranteed, but may be empty.
*/
...
/**
Checks for version 10 by reading the file and checking the length of the first data record.
The total length for version 12+ is 55 characters and for version 10 is 45.
Therefore, a data record length of 50 is considered version 12 (not version 10).
...
private static boolean isVersion_10( String filename ) throws IOException {
	...
	// The last line read above should be the header line with the period.
	// Ignore it because there is some inconsistency in formatting and read another line.
	// For really old files, a header line will not be used.
...
throws Exception {
	String routine = StateCU_CropPatternTS.class.getSimpleName() + ".readStateCUFile";
	String iline = null;
	...
	// TODO SAM 2007-03-04 Need to figure out what version the addition of period to the header occurred.  It seems to have been in version 10 and later?
	...
	//	year1_req = -1,		// Requested first year to process (null requested date = read all).
	//	year2_req = -1;		// Requested last year to process.

	if ( !period_in_header ) {
		// Version 10 and older files do not have header information with the period for the time series.
		// Therefore, grab a reasonable amount of the start and end of the file and then read lines
		// (broken by line breaks) until the last data line is encountered.
		...
		// Skip to 5000 bytes from the end.  This should get some actual data lines.  Save in a temporary array in memory.
		if ( length >= 5000 ) {
			ra.seek ( length - 5000 );
		}
		ra.read ( b );
		...
		// Start at the second record because it is likely that a complete record was not found.
		size = v2.size();
	...
	while ( (iline = in.readLine()) != null ) {
		...
		// If the dates have not been determined, do so (assume that the first line is header with the period, etc.).
		//if ( !version10 && date1_file == null ) {
		if ( date1_file == null ) {
			// Treat all as strings for initial read.
			// Header format is the following, although start and end months and the year type are ignored.
		...
		if ( iline.charAt(0) == ' ' ) {
			...
			// Need to optimize code here to quit reading if requested period is done or have not reached the start of the requested period.
			// The problem is that the code only saves data when a new year/crop is detected so probably need to always try to process
			// one year before the requested period.
			culoc = ((String)v.get(1)).trim();
			total_area = ((String)v.get(2)).trim();
			// Important... ncrops from the file is not actually used.
			// Instead the space at the beginning of lines is used to indicate crops.
			// This works for Version 10 and newer formats.
			ncrops = 0;
			if ( year == year1 ) {
				// Create an object for the CU Location.
				// It is assumed that a structure is listed for each year, even if it has zero crops for a year.
...
@param req_tsident Identifier for requested item series.
If null, return all new time series in the vector.  If not null, return the matching time series.
...
/**
Remove all the time series from the object.
This can be used, for example, when resetting the time series to override a read.
*/
...
Trying to set data outside the period will cause the value to be ignored (the period will not be extended).
...
@exception Exception if there is an error setting the data (e.g., the time series for the crop cannot be found).
*/
public void setCropArea ( String cropName, int year, double area )
throws Exception {
	// Set the value with no flag or description.
	setCropArea ( cropName, year, area, null );
}
...
@param area Area for the crop, for the given year.
@param flag to set
@param flag description to set (the flag description should be set by calling setFlagDescription())
@exception Exception if there is an error setting the data (e.g., the time series for the crop cannot be found).
*/
public void setCropArea ( String cropName, int year, double area, String flag )
throws Exception {
	if ( (year < __date1.getYear()) || (year > __date2.getYear()) ) {
		return;
	}
	// First find the time series.
	YearTS yts = getCropPatternTS ( cropName );
	if ( yts == null ) {
		throw new Exception ( "Unable to find time series for \"" + cropName + "\"" );
	}
	// Set the data value.
	__temp_DateTime.setYear ( year );
	...
	if ( (flag != null) && !flag.isEmpty() ) {
		// Set with flag.
		yts.setDataValue ( __temp_DateTime, area, flag, -1 );
		yts.getDataFlagMetadataList();
	}
	else {
		// Set with no flag.
		yts.setDataValue ( __temp_DateTime, area );
	}
	// Reset the total by adjusting the old value (this performs better than looping through the crop time series).
...
A later reset of the zero can occur without issue.
However, leaving the value as -999 may result in unexpected filled values later.
The total for the year is also set to either 0 (setAllToZero=true) or the total of non-missing values (setAllToZero=false).
...
/**
 * Set the time series flag description, used when data flags are set on data values.
 * This should be called once rather than for each data value set.
 * @param flag flag that is used
 * @param description description for the flag
 */
public void setDataFlagDescription ( String flag, String description ) {
	// Set the description on each of the time series.
	if ( (flag == null) || flag.isEmpty() ) {
		return;
	}
	...
	TSDataFlagMetadata dataFlagMetadata = new TSDataFlagMetadata(flag, description);
	// Add to all of the time series.
	for ( TS ts : this.__tslist ) {
		ts.addDataFlagMetadata(dataFlagMetadata);
	}
}
...
/**
Set the pattern for crops for a given year by specifying a total area and distribution in fractions.
If the year is outside the period, then no action occurs.
@param year Year for the crop data.
@param ncrops Number of crops that should be processed.
...
/**
Set the total crop area.
This will adjust the areas of each crop proportionally, using the initial fractions.
This method is usually only called when data are being reset based on some knowledge of the total acres.
If the old total is zero, all the crops will be zero to zero area.  This method
should NOT be called by other methods that result in a call to this method or an infinite loop will occur.
...
private void sortTimeSeriesByCropName () {
	...
	this.__tslist.clear();
	// Loop through the temporary list and find the first in the alphabetical list, then set in the original list.
...
Only the raw time series are returned.  Use the overloaded version to also return total time series.
@return a list containing all the time series in the data list.
...
/**
Return a list containing all the time series in the data list.
Optionally, process the time series in the instance and add total time series by location and for the entire data set.
This is a performance hit but is useful for summarizing the data.
Any non-zero value in the individual time series will result in a value in the total.
Missing for all time series will result in missing in the total.
The period for the totals is the overall period from all StateCU_CropPatternTS being processed.
@return a list containing all the time series in the data list.
@param dataList A list of CropPatternTS.
@param include_location_totals If true, include totals for each location, equal to the sum of the acreage for all crops.
...
public static List<TS> toTSList ( List<StateCU_CropPatternTS> dataList, boolean include_location_totals,
	...
	String routine = StateCU_CropPatternTS.class.getSimpleName() + ".toTSList";
	...
	for ( int i = 0; i < size; i++ ) {
		...
		if ( include_location_totals ) {
			// Insert a new time series with the total acreage for the location.  Add after the time series for the location.
			crop_name = "AllCrops";
			yts = new YearTS ();
			try {
				TSIdent tsident = new TSIdent (
			...
			yts.setDate2Original(new DateTime(end_DateTime));
			yts.allocateDataSpace();
			// No need to add because the totals are maintained with the data.  Just assign the values.
	...
	if ( include_dataset_totals ) {
		// Insert the time series with the total acreage for the data set, by crop and overall total.
		// Do this after all other time series have been added.  Also reset the description to be more concise.
		for ( j = 0; j < ndistinct_crops; j++ ) {
...
/**
Translate the crop name from the current value to a new value.
The time series identifiers are adjusted.
This is used by StateDMI to correct crop names in input to those used in the DSS.
If a match of the old_crop_name is not found, then nothing occurs.
...
@exception Exception if there is an error (generally only if add fails when the new name is the same as an existing name).
*/
public void translateCropName ( String old_crop_name, String new_crop_name )
throws Exception {
	String routine = getClass().getSimpleName() + ".translateCropName";
	int size = 0;
	if ( __cropNameList != null ) {
		size = __cropNameList.size();
	}
	...
	int existing_crop_pos = -1;	// Position matching new crop name, will be >= 0 if the new name already is in use.
	for ( int i = 0; i < size; i++ ) {
	...
	for ( int i = 0; i < size; i++ ) {
		...
		if ( ts.getIdentifier().getSubType().equalsIgnoreCase( old_crop_name) ) {
			// This crop needs to be renamed, but only do so if not merging.
			// If merging keep the same name so the add comments make sense.
			if ( existing_crop_pos < 0 ) {
	...
	if ( existing_crop_pos >= 0 ) {
		// Need to add the translated crop to the existing crop and remove the translated crop.
		// The overall crop totals will remain the same so there is no need to recompute the totals.
...
/**
Write a list of StateCU_CropPatternTS to a DateValue file.
The filename is adjusted to the working directory if necessary using IOUtil.getPathUsingWorkingDir().
@param filename_prev The name of the previous version of the file (for processing headers).  Specify as null if no previous file is available.
...
/**
Write a List of StateCU_CropPatternTS to a CDS file.
The filename is adjusted to the working directory if necessary using IOUtil.getPathUsingWorkingDir().
@param filename_prev The name of the previous version of the file (for processing headers).
...
@param filename The name of the file to write.
@param dataList A list of StateCU_CropPatternTS to write.
@param new_comments Comments to add to the top of the file.
Specify as null if no comments are available.
@param write_crop_area If true, then the acreage for each crop is shown in addition to the fractions.
...
throws IOException {
	...
	// Make sure that the time series are refreshed before writing.
	// The totals are needed to calculate percentages.
	for ( i=0; i<num; i++ ) {
		cds = dataList.get(i);
		cds.refresh();
	}
		...
		for ( i=0; i<num; i++ ) {
			...
			for ( icrop = 0; icrop < ncrops; icrop++ ) {
				...
				*/
				...
				// Write the area.
				if ( WriteCropArea_boolean ) {
					// TODO SAM 2004-07-28 May need to evaluate whether there needs to be an option of how to write the crop area.
					// For now, calculate to 3 significant figures since that is what the fraction is.
