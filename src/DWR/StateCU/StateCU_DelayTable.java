...
delay entries are added to the end of the previously stored delays.  Returns
...
{	PrintWriter	out = null;
	...
	out.println(cmnt
		+ " *******************************************************");
	out.println(cmnt
		+ " StateCU Delay (Return flow) Table");
	out.println(cmnt);
	out.println(cmnt
		+ "     Format (a8, i4, (12f8.2)");
	out.println(cmnt);
	...
	for (int i = 0; i < ndly; i++) {
		...
		for ( j = 0; j < nvals; j++ ) {
			b.append ( StringUtil.formatString(
				delay.getRet_val(j), m_format) );
			printed = false;
			if ( ((j + 1)%12) == 0 ) {
				// Print the output and initialize a new
				// line...
