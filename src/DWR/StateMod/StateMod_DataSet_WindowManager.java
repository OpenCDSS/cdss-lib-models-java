...
/**
<p>
The StateMod_DataSet_WindowManager class opens/manages/closes display windows for StateMod data set component data.
Currently, only the main windows (e.g., Diversion, Reservoir, but not the secondary windows)
are managed and only one of each main window is allowed to be open at a time.
In the future, secondary windows may also be managed more closely but currently more than one
copy of secondary windows can be opened at the same time.
</p>
<p>
An instance of the window manager also helps with window coordination,
such as initiating a request to display (annotate) a data object on the map or network.
</p>
*/
...
private final int
	CLOSED = 0,
	OPEN = 1;
	// INVISIBLE - might be needed if decide to not fully destroy
	// windows - that is why an integer is tracked (not a boolean).

/**
Windows numbers, managed by the StateMod_GUI.
List in the order of the data set components, although there is not a one to one correspondence.
...
/**
Close the window and set its reference to null.
If the window was never opened, then no action is taken.
Use setWindowOpen() when opening a window to allow the management of windows to occur.
...
private JFrame displayDataSetSummary ( StateMod_DataSet dataset ) {
	...
	// Because the window is a simple ReportJFrame,
	// it will not know how to set its "close" status in this window manager,
	// so need to listen for a close event here.
	f.addWindowListener ( this );
	return f;
}

/**
Show the indicated window type and allow editing.
If that window type is already displayed, bring it to the front.  Otherwise create the window.
...
// TODO - how to deal with printStatus and other calls that need the StateModGUI_JFrame - for now comment out the calls.
/**
Show the indicated window type.
If that window type is already displayed, bring it to the front.  Otherwise create the window.
@param window_type See WINDOW_*.
...
public JFrame displayWindow ( int window_type, boolean editable ) {
	String routine = getClass().getSimpleName() + ".displayWindow";
	//JGUIUtil.setWaitCursor(this, true);
	JFrame win = null;

	if ( getWindowStatus(window_type) == OPEN ) {
		// Window_type already opened so make sure that it is not minimized and move to the front.
	...
	// List windows in the order of the menus (generally).
		...
		// Don't have a special window.
		// Just display all precipitation time series as a graph.
		// This usually works because there are not that many precipitation stations.
		// If it becomes a problem, add additional logic.
		...
		// Don't have a special window.
		// Just display all evaporation time series as a graph.
		// This usually works because there are not that many precipitation stations.
		// If it becomes a problem, add additional logic.
		...
		try {
			...
			// Use a window listener to know when the window closes so that it can be managed like all the other data windows.
			win.addWindowListener ( this );
		}
		catch ( Exception e ) {
			Message.printWarning ( 1, routine, "Error displaying precipitation data." );
...
Currently this occurs by closing the window and then opening it again.
In the future, individual windows may be made more intelligent so that a full refresh of the contents is not necessary.
For now the close/open sequence should work, and is the same approach taken in the previous version of the GUI.
@param win_type The window type to refresh (see WINDOW_*).
@param always_open If true, the window will always be opened.
If false, no action will occur if the window was not originally opened.
@return the JFrame that was refreshed.
This is used, for example, to move the window to the front (but it is not automatically moved to the front by this method).
*/
...
public void renderGeoViewAnnotation ( GeoViewJComponent geoview, Object objectToRender, String label ) {
	...
	if ( objectToRender instanceof StateMod_OperationalRight ) {
		...
		if ( pointDest != null ) {
			GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, pointDest.getX(), pointDest.getY(), 32.0, 0, 0 );
			GRDrawingAreaUtil.drawText(da, labelDest, pointDest.getX(), pointDest.getY(), 0.0, GRText.CENTER_X|GRText.TOP );
		}
		if ( pointSource1 != null ) {
			GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, pointSource1.getX(), pointSource1.getY(), 32.0, 0, 0 );
		...
		if ( pointSource2 != null ) {
			GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, pointSource2.getX(), pointSource2.getY(), 32.0, 0, 0 );
		...
		if ( pointUpstream != null ) {
			GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, pointUpstream.getX(), pointUpstream.getY(), 32.0, 0, 0 );
		...
		if ( (pointDownstream != null) && (pointDownstream != pointUpstream) ) {
			GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, pointDownstream.getX(), pointDownstream.getY(), 32.0, 0, 0 );
		...
		GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.PUSHPIN_VERTICAL, point.getX(), point.getY(), 32.0, 0, 0 );
...
public void renderStateModNetworkAnnotation ( StateMod_Network_JComponent network,
	...
	GRDrawingAreaUtil.setFont(da, "Helvetica", "Bold", 14.0 );
	// Use blue so that red can be used to highlight problems.  The color should allow black symbols on top to be legible.
	...
	if ( objectToRender instanceof StateMod_OperationalRight ) {
		...
		if ( nodeDest != null ) {
			...
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, nodeDest.getX(), nodeDest.getY(),
		...
		if ( nodeSource1 != null ) {
			...
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, nodeSource1.getX(), nodeSource1.getY(),
		...
		if ( nodeSource2 != null ) {
			...
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, nodeSource2.getX(), nodeSource2.getY(),
		...
		if ( metadata.getRightTypeUsesInterveningStructuresWithoutLoss() ||
			...
			for ( String structureID : structureIDList ) {
				...
				if ( nodeIntervening != null ) {
					...
					GRDrawingAreaUtil.drawSymbol(da, GRSymbolShapeType.CIRCLE_FILLED,
		...
		if ( nodeUpstream != null ) {
			...
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, nodeUpstream.getX(), nodeUpstream.getY(),
		...
		if ( (nodeDownstream != null) && (nodeDownstream != nodeUpstream) ) {
			...
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, nodeDownstream.getX(), nodeDownstream.getY(),
		...
		if ( commonID != null ) {
			...
			//GRDrawingAreaUtil.drawSymbol(da, GRSymbol.SYM_FCIR, node.getX(), node.getY(), size, 0, 0);
			//GRDrawingAreaUtil.drawText(da, label, node.getX(), node.getY(), 0.0, GRText.CENTER_X|GRText.BOTTOM );
			GRDrawingAreaUtil.drawSymbolText(da, GRSymbolShapeType.CIRCLE_FILLED, node.getX(), node.getY(), size, label, 0.0, GRText.CENTER_X|GRText.BOTTOM, 0, 0);
		}
	}
}
...
/**
Indicate that a window is opened, and provide the JFrame corresponding to the window.
This method should be called to allow the StateMod GUI to track windows
(so that only one copy of a data set group window is open at a time).
...
public void setWindowOpen(int win_type, JFrame window) {
	String routine = getClass().getSimpleName() + ".setWindowOpen";
	setWindow(win_type, window);
	setWindowStatus(win_type, OPEN);
	Message.printStatus ( 1, routine, "Window set open: " + win_type );
}
...
This is primarily used to update the status of the main JFrame,
with the title and menus being updated if the data set has been modified.
*/
...
// TODO SAM 2006-08-16 Perhaps all windows should be listened for here to simplify the individual window code.
/**
Responds to window closed events.
The following windows are handled because they use generic components that cannot specifically set their close status:
<pre>
Data Set Summary
