...
time paint() has been called.  This should be false when the class is 
instantiated.
*/
...
/**
Whether annotations were read from a file and will need processed before 
being drawn.
*/
...
/**
Whether to draw a red bounding box around annotation text.  Useful for
debugging.
*/
...
/**
A buffered image to use for rendering what will be printed or saved to 
an image file.
*/
...
/**
The height of the drawing area, in pixels.  Used for determining when the
screen has been resized.
*/
private double __drawingAreaHeight = 0;

/**
The width of the drawing area, in pixels.  Used for determining when the
screen has been resized.
*/
...
the network is taller than the paper, then the height is the height of the
entire network.
*/
...
public void actionPerformed(ActionEvent event) {
	...
	if (action.equals(__MENU_ADD_ANNOTATION)) {
		...
		HydroBase_Node.setDrawText(__drawNodeLabels);
		forceRepaint();
	}
		...
		if (__isAnnotation) {
			...
			new StateMod_Network_NodeProperties_JDialog(
				__parent, __nodes, __popupNodeNum);
...
protected void addAnnotation(double x, double y) {
	...
	// escape all instances of " -- they can cause problems when 
	// put into a PropList.
...
boolean isBaseflow, boolean isImport) {
	...
	HydroBase_NodeNetwork network = getNetwork();
...
private void buildNodeArray() {
	...
	while (!done) {
		if (node == null) {
			...
			if (node.getType() != HydroBase_Node.NODE_TYPE_UNKNOWN){
				nodes.add(node);
			}
			holdNode = node;	
...
private void buildSelectedNodesLimits() {
	...
	// created above with the respective nodes' limits and starting X and 
	// Y positions.  
...
protected void calculateDataLimits() {
	...
	if (dWidth >= dHeight) {
		...
		// size __totalDataHeight to be proportional to 
		// __totalDataWidth, given the screen proportions
		__totalDataHeight = 
			((double)__totalBufferHeight 
			/ (double)__totalBufferWidth)
			* __totalDataWidth;
		...
		if (__dataLimits.getHeight() > __totalDataHeight) {
			...
			// reset the total data height to be the height of
			// the data limits.  
		...
		__dataBottomY = __dataLimits.getBottomY();
		// size __totalDataWidth to be proportional to 
		// __totalDataHeight, given the screen proportions		
		__totalDataWidth = 
			((double)__totalBufferWidth 
			/ (double)__totalBufferHeight)
			* __totalDataHeight;
...
*/
private int calculateScaledFont(String name, String style, int size, 
boolean isPrinting) {
	double scale = 0;
	if (__fitWidth) {
...
will be appended to the node's ID.   The node is guaranteed to have a 
unique id by the end of this method.
...
*/		
	...
	node.newNode(HydroBase_Node.NODE_TYPE_OTHER);
	...
	node.setY(node.getY() - yInc);
	node.newNode(HydroBase_Node.NODE_TYPE_FLOW);
	node.draw(__drawingArea);
	...
	node.setY(node.getY() - yInc);
	node.setX(col1x);
	node.newNode(HydroBase_Node.NODE_TYPE_RES);
	node.draw(__drawingArea);
	...
	node.setY(node.getY() - yInc);
	node.setX(col1x);
	node.newNode(HydroBase_Node.NODE_TYPE_ISF);
	node.draw(__drawingArea);
	...
	node.setY(node.getY() - yInc);
	node.setX(col1x);
	node.newNode(HydroBase_Node.NODE_TYPE_DIV_AND_WELL);
	node.draw(__drawingArea);
	...
	node.setY(node.getY() - yInc);
	node.setX(col1x);
	node.newNode(HydroBase_Node.NODE_TYPE_WELL);
	node.draw(__drawingArea);
	...
	node.setY(node.getY() - yInc);
	node.setX(col1x);
	node.newNode(HydroBase_Node.NODE_TYPE_DIV);
	node.draw(__drawingArea);
...
private void drawLinks() {
	...
	int size = __links.size();
	PropList p = null;
	HydroBase_Node node1 = null;
	HydroBase_Node node2 = null;
	String id1 = null;
	String id2 = null;
	for (int i = 0; i < size; i++) {
		p = (PropList)__links.elementAt(i);
...
private void drawNetworkLines() {
	...
	HydroBase_Node dsRealNode = null;
	HydroBase_Node holdNode = null;
	HydroBase_Node holdNode2 = null;
	HydroBase_Node node = null;
	HydroBase_Node nodeTop = __network.getMostUpstreamNode();
	...
	for (node = nodeTop; node != null; 
		...
		// connections are only between visible nodes
		holdNode2 = ds;
		while (ds.getType() == HydroBase_Node.NODE_TYPE_UNKNOWN) {
...
position.
*/
protected HydroBase_Node getAnnotationNode(int nodeNum) {
...
*/
protected HydroBase_NodeNetwork getNetwork() {
	return __network;
}
...
*/
protected HydroBase_Node[] getNodesArray() {
	return __nodes;
}
...
public void paint(Graphics g) {
	...
	// the network is being printed, it is only drawn to the 
	// BufferedImage one time.
	if (__printCount > 0) {
		return;
	}
	...
	if (!__initialized) {
		...
		// and that doesn't even BEGIN to get into the problems with
		// points vs. pixels ...
	...
	// one of them is set during a repaint it will apply them to the
	// gui settings.
	...
	if (__forceRefresh) {
		...
		setAntiAlias(__antiAlias);		
		if (__drawInchGrid) {
			// change the limits so that the drawing is done
			// in device units, not data units
	...
	if (!__printingNetwork && !__printingScreen && !__savingNetwork
		...
		// return here because nodes can't ever be in drag when
		// printing occurs.
...
*/
protected void readMakenetFile(HydroBaseDMI dmi, String filename) {
	__network = new HydroBase_NodeNetwork();
	__network.readMakenetNetworkFile(dmi, filename, true);
...
*/
protected void setNetwork(HydroBase_NodeNetwork network, boolean dirty, 
boolean doAll) {
...
protected void setPrintNodeSize(double size) {
	...
	HydroBase_Node.setIconDiam((int)(size));
	double diam = 0;
	if (__fitWidth) {
		diam = convertX(size);
	}
...
be updated.
*/
protected void updateAnnotation(int nodeNum, HydroBase_Node node) {
	PropList p = (PropList)node.getAssociatedObject();
...
*/
private boolean within(GRLimits rect, HydroBase_Node node) {
	double x = node.getX();
	double y = node.getY();
...
{	String routine = "StateMod_Network_JComponent.writeListFiles";
	...
	int[] types = {
		-1,					// All nodes
		HydroBase_Node.NODE_TYPE_FLOW,		// Stream gage
		HydroBase_Node.NODE_TYPE_DIV,		// Diversion
		HydroBase_Node.NODE_TYPE_DIV_AND_WELL,	// Diversion + Well
		HydroBase_Node.NODE_TYPE_RES,		// Reservoir
		HydroBase_Node.NODE_TYPE_ISF,		// Instream flow
		HydroBase_Node.NODE_TYPE_WELL,		// Well
		...
		//HydroBase_Node.NODE_TYPE_BASEFLOW,	// Stream estimate
		HydroBase_Node.NODE_TYPE_OTHER		// Not other stations
	};
		...
		if (v != null && v.size() > 0) {
			...
			if ( types[i] == -1 ) {
				...
				comments[0] = "The following list contains " +
					...
					types[i], HydroBase_Node.ABBREVIATION) +
					" (" +
					HydroBase_Node.getTypeString(
					types[i], HydroBase_Node.FULL) + ")";
			...
			try {
				HydroBase_NodeNetwork.writeListFile(
