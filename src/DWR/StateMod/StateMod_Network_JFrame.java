...
*/
public StateMod_Network_JFrame(HydroBase_NodeNetwork network) 
throws Exception {
	super();
	__newNetwork = false;
	double scale = .5;
...
*/
public StateMod_Network_JFrame(HydroBaseDMI dmi, String filename)
throws Exception {
	super();
	initializeExistingNetwork(dmi, filename);
}
...
throws Exception {
	super();
	if (newNetwork) {
		initializeNewNetwork(dmi, filename);
	}
	else {
		initializeExistingNetwork(dmi, filename);
	}
...
*/
public void displayNode(HydroBase_Node node, int nodeNum) {
	displayNode(node);
}
...
*/
public HydroBase_NodeNetwork getNetwork() {
...
*/
private void initializeExistingNetwork(HydroBaseDMI dmi, String filename)
throws Exception {
	__dmi = dmi;
	__newNetwork = false;
	__filename = filename;
	...
	boolean isXML = HydroBase_NodeNetwork.isXML(filename);

	if (!isXML) {
		__device.readMakenetFile(__dmi, filename);
	}
	else {
		__isXML = true;
	}	
...
@throws Exception if an error occurs when initializing.
*/
private void initializeNewNetwork(HydroBaseDMI dmi, String filename)
throws Exception {
	__dmi = dmi;
	__isXML = true;
	__newNetwork = true;
	...
	HydroBase_NodeNetwork network = new HydroBase_NodeNetwork(true);
	__device.setNetwork(network, false, true);
	Vector v = network.getNodesForType(HydroBase_Node.NODE_TYPE_END);
	// change the "5" to something better if a different default paper
	// size than "C" is used.
	((HydroBase_Node)v.elementAt(0)).setX(__rx / 4.3);
	((HydroBase_Node)v.elementAt(0)).setY(__ty / 5);
...
throws Exception {
	...
	HydroBase_NodeNetwork network = 
		HydroBase_NodeNetwork.readXMLNetworkFile(filename);
	__device.setNetwork(network, false, true);
...
*/
private void setBoundsFromNetwork(HydroBase_NodeNetwork network) {
	__rx = network.getRX();
	__ty = network.getTY();
	__lx = network.getLX();
	__by = network.getBY();
...
//	IOUtil.testing(true);
	...
	if (!__isXML) {
		HydroBase_NodeNetwork network = __device.getNetwork();

		HydroBase_Node node = null;
		HydroBase_Node nodeTop = network.getMostUpstreamNode();
