...
*/
public StateMod_ReservoirAreaCap_Data_JFrame(Vector data, String titleString,
boolean editable)
throws Exception {
	super();
	...
	for (int i = 0; i < size; i++) {
		...
		for (j = 0; j < size2; j++) {
			a = (StateMod_ReservoirAreaCap)areacaps.elementAt(j);
			a.setCgoto(r.getID());
		    	v.add(a);
		}
	}	
...
protected void apply() {
	...
	int size = _data.size();
	for (int i = 0; i < size; i++) {
		ac = (StateMod_ReservoirAreaCap)_data.elementAt(i);
		ac.createBackup();
	}
}
...
throws Exception {
	StateMod_ReservoirAreaCap_Data_TableModel tableModel 
		= new StateMod_ReservoirAreaCap_Data_TableModel(_data, 
		_editable);
