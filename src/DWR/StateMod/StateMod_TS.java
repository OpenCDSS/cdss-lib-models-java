...
/**
Use the following to specify that precision of output should be determined
from the units file.
*/
...
{	String message = null, routine = "StateMod_TS.getFileDataInterval";
	...
	try {	iline = ifp.readLine();
		...
		// Now should have the header.  Read one more to get to the
		// data...
		iline = ifp.readLine();
		if ( iline == null ) {
			throw new Exception ( "end of file" );
		}
		// Should be first data line.  If longer than the
		// threshold, assume daily.
		if ( iline.length() > 150 ) {
			iline = null;
			full_filename = null;
			ifp.close();
...
{	// If req_precision is > PRECISION_SPECIAL_OFFSET, divide to get the
	...
	if (	(req_precision > PRECISION_SPECIAL_OFFSET) ||
		(req_precision*-1 > PRECISION_SPECIAL_OFFSET) ) {
	...
	else {	// Need to compute the largest number.  Using the example above,
	...
	else {	plus_value = value*-1.0;
	}
	if ( plus_value <= largest_number ) {
		// It fits within the precision.  Return the positive
...
requested precision.  If negative, use the requested precision (positive value)
if the resulting number fits into the width of the column.  Otherwise adjust
the precision to fit.  If PRECISION_USE_UNITS, use the units to determine
the precision.
*/
public static int getPrecision ( int req_precision, int width, double value,
				String units )
{	// if no units data, call the simple version...
	...
	try {	DataFormat units_format = DataUnits.getOutputFormat(
				units, width);
		__last_units = units;
		__last_units_precision = units_format.getPrecision();
	}
	catch ( Exception e ) {
		// Could not get units so return the requested precision without
		// checking units...
		return getPrecision ( req_precision, width, value );
	}

	// Now have the units precision so call the general routine with a
	// negative value...

	if ( units_precision < 0 ) {
		return getPrecision ( units_precision, width, value );
	}
	else {	return getPrecision ( -1*units_precision, width, value );
	}
}
...
programmer knows what sample is supported.  Perhaps they could be stored in
...
{	TS	ts = null;
	...
	// Determine the interval of the file and create a time series that
	// matches...
...
{	int	dl = 40, i, line_count = 0, m1, m2, y1, y2,
	...
	if ( iline.charAt(3) == '/' ) {
		...
		"Non-standard header for file \"" +
		full_filename + "\" allowing with work-around." );
	...
	else {	date1_header = new DateTime ( DateTime.PRECISION_MONTH );
		date1_header.setYear ( y1 );
		date1_header.setMonth ( m1 );
	}
	...
	else {	date2_header = new DateTime ( DateTime.PRECISION_MONTH );
		date2_header.setYear ( y2 );
		date2_header.setMonth ( m2 );
	}
	String units = ((String)v.elementAt(4)).trim();
	...
	if ( y1 == 0 ) { // average monthly series
		standard_ts = false;
		if ( Message.isDebugOn ) {
			Message.printDebug ( dl, routine,
			"Found average monthly series" );	
		}

		format[0] = StringUtil.TYPE_STRING;	// Year not used.
		current_year = 0;			// Start year will be
							// calendar year 0
		init_year = 0;
		if ( m2 < m1 ) {
			y2 = 1;				// End year is calendar
							// year 1.
		}
	}
	else {	// standard time series, includes a year on input lines
		if ( Message.isDebugOn ) {
			Message.printDebug ( dl, routine,
			"Found time series" );	
		}
	...
	boolean single_ts = false;	// Indicates whether a single time
					// series is in the file.
	...
	while ( true ) {
		if ( data_line_count == 0 ) {
			...
			if ( iline == null ) {
				...
				// Comment line.  Count the line but do not
				// treat as data...
				++line_count;
				continue;
			}
			...
			// be different (but always return the file contents),
			...
			if ( second_iline != null ) {
				...
				// identifiers are the same.  If so, assume one
				// time series in the file...
				int line1_year = StringUtil.atoi(
					iline.substring(0,5).trim() );
				int line2_year = StringUtil.atoi(
					second_iline.substring(0,5).trim() );
				String line1_id = iline.substring(5,17).trim();
				String line2_id =
					second_iline.substring(5,17).trim();
				if (	line1_id.equals(line2_id) &&
					(line1_year != line2_year) ) {
					single_ts = true;
					if ( Message.isDebugOn ) {
						Message.printDebug ( 1, routine,
						"Single TS detected - reading "+
						"all." );
					}
					if (	(req_id != null) &&
						!line1_id.equalsIgnoreCase(
						req_id) ) {
						Message.printStatus ( 1,routine,
						"Reading StateMod file, the " +
						"requested ID is \"" +
						req_id + "\" but the " +
						"file contains only \"" +
						line1_id + "\"." );
						Message.printStatus ( 1,routine,
						"Will read the file's"+
						" data but use the requested "+
						"identifier." );
					}
				}
			}
			...
			// Special case where the 2nd line was read immediately
			// after the first to check to see if only one time
			// series is in the file.  If here, set the line to
		...
		if (	((file_interval == TimeInterval.DAY) &&
			...
			((file_interval == TimeInterval.MONTH) &&
			(current_year == init_year)) ) {
			if ( req_id == null ) {
				// Create a new time series...
				if ( file_interval == TimeInterval.DAY ) {
				...
				// We want the requested time series to get
				// filled in...
				ts = req_ts;
				req_id_found = true;
				numts = 1;
			...
			if ( Message.isDebugOn ) {
				Message.printDebug ( dl, routine, 
				"Setting data units to " + units );
			}
			ts.setDataUnits ( units );
			ts.setDataUnitsOriginal ( units );

			// The input name is the full path to the input file...
			ts.setInputName ( full_filename );
			// Set other identifier information.  The
			// readTimeSeries() version that takes a full identifier
			// will reset the file information because it knows
			// whether the orginal filename was from the scenario,
			// etc.
			TSIdent ident = new TSIdent ();
			ident.setLocation ( id );
			...
			// May be forcing a read if only one time series but
			// only reset the identifier if the file identifier
			// does match the requested...
			if (	((req_id != null) && req_id_found &&
				id.equalsIgnoreCase(req_id)) ||
				(req_id == null) ) {
			...
			if ( !req_id_found ) {
				// Attach new time series to list.  This is only
				// done if we have not passed in a requested
				// time series to fill.
				if ( tslist == null ) {
		...
		if ( read_data ) {
			if ( file_interval == TimeInterval.DAY ) {
				// Need to loop through the proper number of
				// days for the month...
				ndata_per_line = TimeUtil.numDaysInMonth(
		...
		date = null;
		full_filename = null;
		// Force a garbage collect if the number of time series is more
		// than 1 because of all the intermediate objects used for
		// parsing...
	...
	date = null;
	full_filename = null;
	// Force a garbage collect if the number of time series is more than
	// 1 because of all the intermediate objects used for parsing...
	if ( (tslist != null) && (tslist.size() > 1) ) {
		System.gc();
	}
	return tslist;
...
@param props Properties to control the output.  The following properties are
recognized:
...
{	String rtn = "StateMod_TS.writePatternTimeSeriesList";
	...
	int req_interval_base = TimeInterval.MONTH;
						// The interval for time series
						// passed to this method.  Get
						// from the first non-null time
						// series - later may pass in.
	...
	int interval_base;	// Used to check each time series against the
				// interval for the file, which was determined
				// above
	...
	for ( int i=0; i<nseries; i++ ) {
		...
		if ( interval_base != req_interval_base ) {
			...
			if ( req_interval_base == TimeInterval.MONTH ) {
				...
				"A TS interval other than monthly " +
	...
	PrintWriter out = IOUtil.processFileHeaders (
		...
		comment_str, ignore_str, 0 );
	if ( out == null ) {
		Message.printWarning ( 2, rtn,
		"Error writing time series to \"" + full_filename + "\"" );
		throw new Exception ( 
		"Error opening output file \"" + full_filename + "\"" );
	}
	...
	else {	// if ( output_format.equalsIgnoreCase ("CYR" ))
	...
	String	empty_string = "-", tmpdesc, tmpid, tmplocation, tmpsource,
		tmptype, tmpunits;
	...
	for ( int i=0; i < nseries; i++ ) {
		...
		if ( Message.isDebugOn ) {
			...
			"Processing time series [" + i+ "] \"" + tmpid + "\"" );
		}
		if ( tmpid.length() == 0 ) {
			tmpid = empty_string;
		}
	...
	// before because the comments should be printed even if there is no
	// time series data to print.
	...
	*/
	...
	String yeartype = "WYR";
		...
		if ( (date1 == null) || (date2 == null) ) {
			try {	TSLimits valid_dates =
				...
				if ( (month1==0) || (year1==0) ) {
					req_date1.setMonth ( 
					valid_dates.getDate1().getMonth());
					req_date1.setYear ( 
					valid_dates.getDate1().getYear());
				}
				if ( (month2==0) || (year2==0) ) {
					req_date2.setMonth ( 
					valid_dates.getDate2().getMonth());
					req_date2.setYear ( 
					valid_dates.getDate2().getYear());
				}
			} catch ( Exception e ) {
				Message.printWarning ( 2, rtn, 
		...
		// agree with output_format (e.g., if "WYR" requested but
		// req_date1 != 10)
		if ( output_format.equalsIgnoreCase ( "WYR" )) {
		...
		format = "   %2d/%4d  -     %2d/%4d%5.5s" +
			StringUtil.formatString ( yeartype,"%5.5s");
	...
	Message.printStatus ( 2, rtn,
		"Writing time series data for " + req_date1 + " to " +
		req_date2 );
	DateTime date = new DateTime ( DateTime.PRECISION_MONTH );
	...
	*/
	...
	if ( req_interval_base == TimeInterval.MONTH ) {
		...
		for ( ; date.lessThanOrEqualTo(req_date2); date.addMonth(12)) {
			...
			for ( j = 0; j < nseries; j++ ) {
				...
				// it until ready to println to the output
				// file...
				if ( !include_ts[j] ) {
					continue;
				}
	...
	/* TODO SAM 2005-05-06 maybe enable daily later.
		...
		// output.
		// The outer loop interates on months...
		int monthly_count = 0;
		...
		// than the monthly formats - trailing "." is enforced - not
		// sure why?
		...
		for ( ; date.lessThanOrEqualTo(req_date2); date.addMonth(1)) {
			for ( j = 0; j < nseries; j++ ) {
				...
				// for every month (months containing fewer than
				// 31 days use 0s as fillers).
				ndays = TimeUtil.numDaysInMonth ( 
					cdate.getMonth(), cdate.getYear());
				for ( day=1; day <=31; day++) {
					if ( day <= ndays ) {
						cdate.setDay ( day );
						value = tsptr.getDataValue (
							cdate);
					}
					else {	value = 0.0;
					}
					precision = getPrecision ( 
							req_precision,8,value );
					iline_format_buffer.append(
						format8_for_precision[
						precision]);
					if (tsptr.isDataMissing (value))
					{	// Missing data so don't add to
						// the annual value.  Print
				...
				precision = getPrecision ( req_precision, 10, 
					monthly_sum );
				iline_format_buffer.append (
...
@param req_precision Requested precision of output (number of digits after the
decimal
...
{	String rtn	= "StateMod_TS.writeTimeSeriesList";
	...
	// Verify that the ts vector is not null.  Then
	...
	// above because to make sure that the requested interval is determined
	// first.
	...
	for (int i=0; i<nseries; i++ ) {
		...
		// becausing mixing a monthly average with normal monthly data
	...
	if (	output_units.equalsIgnoreCase("AF") ||
		output_units.equalsIgnoreCase("ACFT") ||
		output_units.equalsIgnoreCase("AF/M") ||
		output_units.equalsIgnoreCase("IN") ||
		output_units.equalsIgnoreCase("MM") ) {
		// Assume only these need to be totaled...
		Message.printStatus ( 1, rtn,
		"Using total for annual value, based on units \"" +
		output_units + "\"" );
		do_total = true;
		year_title = "Total";	// Also used as "month_title" for daily
	}
	else {	Message.printStatus ( 1, rtn,
		"Using average for annual value, based on units \"" +
		output_units + "\""  );
		do_total = false;
		year_title = "Average";
	}
		...
		for ( ; date.lessThanOrEqualTo(req_date2); date.addMonth(12)) {
			...
			for ( j = 0; j < nseries; j++ ) {
				...
				if ( req_precision == PRECISION_USE_UNITS ) {
					// Only get the units if we are going to
					// use them...
				...
				// Add total to output, format and print output
				// line
				if ( req_precision == PRECISION_USE_UNITS ) {
					precision = getPrecision (
					req_precision, 10, annual_sum, units );
				}
				else {	precision = getPrecision (
					req_precision, 10, annual_sum );
				}
				iline_format_buffer.append (
					format10_for_precision[precision] );
...
@param MissingDV Value to be printed when missing values are encountered.
@param precision Requested precision of output (number of digits after the
decimal
point).  The default is is 2.  This should be set according to the datatype
in calling routines and is not automatically set here.  The full width for time
series is 8 characters.
@exception Exception if there is an error writing the file.
*/
...
{	String []	comment_str = { "#" }; 
	...
	writeTimeSeriesList (out, tslist, date1, date2, 
		output_format, MissingDV, precision, false );
...
The IOUtil.getPathUsingWorkingDir() method is applied to the filename.
@param tslist List of time series to write, which must be of the same
interval.
...
digits to accomodate large numbers.  This allows some flexibility to fit
large numbers into the standard field widths of StateMod files.
If PRECISION_FROM_UNITS, then the data units are checked to
determine
...
<td>Indicates whether to print time series creation history (true) or
not (false).
</td>
<td>false</td>
</tr>
...
{	String prop_value = null;
	...
	if ( prop_value == null ) {
		...
		if ( prop_value != null ) {
			Message.printWarning ( 2, routine,
			"StateMod write property Start is obsolete.  Use " +
			"OutputStart.");
		}
	}
	if ( prop_value != null ) {
		try {	date1 = DateTime.parse ( prop_value.trim() );
		}
		catch ( Exception e ) {
			Message.printWarning ( 1, routine,
			"Error parsing starting date \"" + prop_value + "\"." +
			"  Using null." );
			date1 = null;
		}
	}
	...
	if ( prop_value == null ) {
		...
		if ( prop_value != null ) {
			Message.printWarning ( 2, routine,
			"StateMod write property End is obsolete.  " +
			"Use OutputEnd.");
		}
	}
	if ( prop_value != null ) {
		try {	date2 = DateTime.parse ( prop_value.trim() );
		}
		catch ( Exception e ) {
			Message.printWarning ( 1, routine,
			"Error parsing ending date \"" + prop_value + "\"." +
			"  Using null." );
			date2 = null;
		}
	}
	...
	String [] newcomments = null;	// Default
	Object prop_contents = props.getContents ( "NewComments" );
	if ( prop_contents != null ) {
		newcomments = (String [])prop_contents;
	}
